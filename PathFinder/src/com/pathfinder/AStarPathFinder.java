...
public class AStarPathFinder implements IPathFinder {
	...
	public static Logger logger = Logger.getLogger(AStarPathFinder.class.getName());
	...
	public Path findPath() throws PathNotFoundException{
		...
		if (map.blocked(tx, ty)) {
			logger.info("The destination is blocked.");
			throw new PathNotFoundException("The destination is blocked.");
		}
			...
			for (int x=-1;x<2;x++) {
				for (int y=-1;y<2;y++) {
					...
					if (isValidLocation(sx,sy,xp,yp)) {
						...
						float nextStepCost = current.cost + getMovementCost(current.x, current.y, xp, yp)+getHeuristicCost(xp, yp, tx, ty);
		...
		if (nodes[tx][ty].parent == null) {
			throw new PathNotFoundException("No path found for given map.");
		}
	...
	private class SortedList {
		...
		//private PriorityQueue<Node> list = new PriorityQueue<Node>();
		/**
